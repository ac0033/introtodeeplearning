...
def abc2wav(abc_file):
    """Convert an ABC file to WAV on Colab/Linux or a local Windows machine."""
    path_to_tool = os.path.join(cwd, "bin", "abc2wav")
    if os.name != "nt" and shutil.which("abc2midi") and shutil.which("timidity"):
        return subprocess.run([path_to_tool, abc_file], check=False).returncode

    midi_file = os.path.splitext(abc_file)[0] + ".mid"
    wav_file = os.path.splitext(abc_file)[0] + ".wav"
    ...
    try:
        ...
        score = music21.converter.parse(abc_file, format="abc")
        score.write("midi", fp=midi_file)
        midi = pretty_midi.PrettyMIDI(midi_file)
        audio = midi.synthesize(fs=44100)
        soundfile.write(wav_file, audio, 44100)
        return 0
    except Exception as exc:
        print("Could not convert ABC notation to audio: {}".format(exc))
        return 1
    finally:
        for temporary_file in (abc_file, midi_file):
